 ...
 * @author Matheus
 */
public final class reporte_bono extends javax.swing.JPanel {
    Dimension d;
    Empleado emp;
    int turno=1;
    ...
    public void llenar(){
        if(!temporal){
            ...
            try{
                ...
                List<Bono> hor = helper.getBonoByTipo(jLabel1.getText());
                helper.cerrarSesion();
                temporal= true;
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Bonos", "Alimentación", "Décimo Tercero Mensualizado", "Décimo Cuarto Mensualizado", "Movilización", "Liquidación de Vacaciones", "Reposiciones", "Liq. Decimotercero Dic-Jul 2015" }));

        jButton1.setText("Actividades Comunes");
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        try{
            ServiceHb helper = new ServiceHb();
            helper.iniciarTransaccion();
            List<Empleado> lt = helper.getPersonasByTipo();
            if(lt!=null){
                for(Empleado i: lt){
                    if(!i.getCuidadResidencia().contains("Quito")){
                        Prestamos p = new Prestamos();
                        p.setAnio(jComboBox27.getSelectedItem().toString());
                        p.setMes(jComboBox21.getSelectedItem().toString());
                        p.setCi(i.getCid());
                        p.setMotivo("Actividades Comunes");
                        p.setObservacion("Descuento de $1,50  por actividades comunes");
                        p.setValor("-1.5");
                        p.setAccion("A");
                        p.setResponsable("Juleissy Iñiga");
                        p.setFecha(Funcion.DateFormatSql(jDateChooser5.getDate()));
