 ...
 * @author homemig
 */
public class fmensual2 extends javax.swing.JPanel {
    String cl;
    String mes;
    String anio;
    String depa="";
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Fecha", "N° Factura", "N° Aut. Factura", "Proveedor", "Subtotal", "Subtotal 0%", "Iva 12%", "Total", "N° Retencion", "N° Aut. Retencion", "Fecha Retencion", "1%", "2%", "8%", "10%", "30%", "70%", "100%"
            }
        ) {
    ...
    private void jButton2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton2MouseClicked
        try{
            ...
            List<Factura> ltfact =null;
            Calendar cal= null;
            Calendar cali= null;
            if(jDateChooser1.getDate()!=null && jDateChooser2.getDate()!=null){
                cal = Calendar.getInstance();
                cal.setTimeInMillis(jDateChooser2.getDate().getTime());
                cal.add(Calendar.DAY_OF_YEAR, 1);
                cal.add(Calendar.SECOND, -1);
                
                cali = Calendar.getInstance();
                cali.setTimeInMillis(jDateChooser1.getDate().getTime());
                cali.add(Calendar.SECOND,-1);
                ltfact= helper.getbusquedaFactura(cali.getTime(),cal.getTime());
            }
            ...
            if(ltfact!=null){
                for(Factura i: ltfact){
                    DefaultTableModel temp = (DefaultTableModel) jTable1.getModel();
                    Proveedores p = (Proveedores) helper.obtenerObjeto(Proveedores.class, Integer.parseInt(i.getIdproveedor()));
                    Retencionfactu ret=null;
                    try{
                        if(i.getIdretencion()!=null){
                            ret = helper.getrentencion2(i.getIdretencion());
                        }
                    }catch(Exception io){
                    ...
                    String s1=df.format(Double.parseDouble(i.getSubtotal())).replace(",", ".");
                    String s2=df.format(Double.parseDouble(i.getSubtotal0())).replace(",", ".");
                    String s3=df.format(Double.parseDouble(i.getIva())).replace(",", ".");
                    String s4=df.format(Double.parseDouble(i.getTotal())).replace(",", ".");
                    if(i.getIdnombre().contains("40127")){
                        System.out.println("");
                    }
                    if(i.getFecha2()!=null){
                        if(cali.getTime().before(i.getFecha2()) && cal.getTime().after(i.getFecha2())){
                            Object nuevo[]= {i.getFecha2(),i.getIdnombre(),i.getSri(),p.getRazon(),s1,s2,s3,s4,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10};
                            temp.addRow(nuevo);   
                        }
                    }else{
                        Object nuevo[]= {i.getFecha(),i.getIdnombre(),i.getSri(),p.getRazon(),s1,s2,s3,s4,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10};
                        temp.addRow(nuevo);   
                    }
                }
            }
    ...
    public boolean export(){ 
        ...
        File file = new File(direccion+"/"+jTextField1.getText()+".xls");
            ...
            try ( //Nuestro flujo de salida para apuntar a donde vamos a escribir
                ...
                String[] ftc= {"FECHA FACTURA","N° FACTURA","N° AUT. FACTURA","PROVEEDOR","SUBTOTAL 12%","SUBTOTAL 0%","IVA 12%","TOTAL","N° RETENCION","N° AUT. RETENCION","FECHA RETENCION","%1","%2","%8","%10","%30","%70","%100","%"};
                ...
                for(int i=0;i< jTable1.getColumnCount();i++){
                    for(int j=1;j<jTable1.getRowCount();j++){
                        ...
                        Label l = new Label(i, j, objeto.toString());
                        s.addCell(l);
                    }
                }
