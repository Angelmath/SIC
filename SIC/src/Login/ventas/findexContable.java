 ...
 * @author homemig
 */
public class findexContable extends javax.swing.JPanel {
    String cl;
    /**
     * Creates new form cliente
     */
    public findexContable() {
    ...
    private void initComponents() {
        ...
        add(jScrollPane1);
        jScrollPane1.setBounds(0, 0, 580, 350);

        jLabel22.setText("Indice:");
        add(jLabel22);
        jLabel22.setBounds(10, 390, 140, 30);

        jLabel23.setText("Valor:");
        add(jLabel23);
        jLabel23.setBounds(10, 430, 140, 30);
        add(jTextField4);
        jTextField4.setBounds(170, 380, 190, 40);
        add(jTextField5);
        jTextField5.setBounds(170, 420, 190, 40);
        ...
        add(jButton11);
        jButton11.setBounds(410, 420, 140, 40);
        add(jLabel2);
        ...
        add(jButton1);
        jButton1.setBounds(410, 380, 140, 40);
    ...
    private void jButton11MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton11MouseClicked
        try{
            ...
            if(jLabel2.getText()!=null){
                index = (Indexcontable)helper.obtenerObjeto(Indexcontable.class,Integer.parseInt(jLabel2.getText()));
            }else{
                index = new Indexcontable();
            }
            index.setMonto(jTextField5.getText());
            index.setIndice(jTextField4.getText());
                       
            helper.crearOrActualizarObjeto(index);
            helper.confirmarTransaccion();
            helper.cerrarSesion();
            JOptionPane.showMessageDialog(null, "Indice Agregado");
            limpiar();
            llenartabla1();
        }catch(Exception io){
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if(col==2){
            ...
            if(selectedSiteName.equalsIgnoreCase("cajamarca")){
                try{
                    ServiceHb helper = new ServiceHb();
                    helper.iniciarTransaccion();
                    Indexcontable p = (Indexcontable)jTable1.getValueAt(row, col);
                    helper.eliminarObjeto(p);
                    helper.confirmarTransaccion();
                    helper.cerrarSesion();
                    JOptionPane.showMessageDialog(null, "Indice Eliminado");
                    llenartabla1();
                }catch(Exception io){
        ...
        if(col==0){
            try{
                ServiceHb helper= new ServiceHb();
                helper.iniciarTransaccion();
                Indexcontable p =(Indexcontable)jTable1.getValueAt(row, 2);                
                helper.cerrarSesion();
                jTextField4.setText(p.getIndice());
        ...
        public void llenartabla1(){
            try{
                ServiceHb helper = new ServiceHb();
                helper.iniciarTransaccion();
                List<Indexcontable> index = helper.getIndexcontable();
                helper.cerrarSesion();
                ...
                if(index!=null){
                    ...
                    for(Indexcontable i: index){
                        DefaultTableModel temp = (DefaultTableModel) jTable1.getModel();
                        Object nuevo[]= {i.getIndice(),i.getMonto(),i};
                        temp.addRow(nuevo);
                    }
                }
            }catch(Exception io){
