...
//    }
    ...
    public void mouseClicked(MouseEvent evt) {
        if(evt.getButton()==MouseEvent.BUTTON3){   
            ...
            Dimension pageSize = currentPage.getUnstretchedSize(sz.width, sz.height,
                    null);

            // get the new image
            currentImage = currentPage.getImage(pageSize.width, pageSize.height,
                    null, this);

            // calculate the transform from screen to page space
            currentXform = currentPage.getInitialTransform(pageSize.width,
                    pageSize.height,
                    null);
            try {
                currentXform = currentXform.createInverse();
